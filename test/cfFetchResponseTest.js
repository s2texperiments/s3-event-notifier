...
describe('cf-fetch-response', () => {
    ...
    it('send success', async () => {
        ...
        const nodeFetchFake = fake.resolves({text: () => 'call succeeded'});

        let underTest = proxyquire('../cfFetchResponse.js', {
    ...
    it('send success but failed because payload object is not stringifyable', async () => {
    ...
    it('send success but call failed', async () => {
        expect(1).to.be.above(2);
        ...
        let result = await underTest.sendSuccess(cfEvent, cfContext);
        console.log(result);
    });
